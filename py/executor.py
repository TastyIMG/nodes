...
logger = logging.getLogger("Tasty.Executor")
...
# ── Simple trigger node for testing ──────────────────────────────

class TriggerFireNode:
    """Fires a trigger signal. Always executes."""
    ALWAYS_EXECUTE = True
    ...
    RETURN_TYPES = ("TRIGGER", "TASTY_JSON")
    RETURN_NAMES = ("trigger", "json_result")
    FUNCTION = "execute"
    CATEGORY = "Tasty/Flow"
    OUTPUT_NODE = True
    ...
    def execute(self, trigger_in=None):
        import json
        json_result = json.dumps({"node": "TriggerFireNode", "trigger_in": str(trigger_in)})
        return ("done", json_result)
...
def _install():
    ...
    logger.info("🔥 Tasty executor override installed — trigger nodes will always re-execute")
...
NODE_CLASS_MAPPINGS = {
    "TriggerFireNode": TriggerFireNode,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "TriggerFireNode": "Trigger Fire",
}
